...
#ifdef _WIN32
#define Win32_LogWarn(fmt, ...)  Win32_Log(LT_Warn,  S(fmt), __VA_ARGS__)
#define Win32_LogError(fmt, ...) Win32_Log(LT_Error, S(fmt), __VA_ARGS__)
#define Win32_LogFatal(fmt, ...) Win32_Log(LT_Fatal, S(fmt), __VA_ARGS__)
...
#define Win32_Check(inFuncCall, inCondicional) \
    do { \
        if ( !(inFuncCall inCondicional) ) { \
            ...
            LogError(S("(windows %u) %s"), lastError, buffer); \
            LocalFree(buffer); \
            DEBUGGER_BREAKPOINT; \
        } \
    } while (0)
