...
#define Mem_Shift_Right_Array(inArray, inStartIndex, inEndIndex, inShiftCount) \
    do { \
        ...
        Mem_Shift_Right(&(inArray)[inEndIndex], ((inEndIndex) - (inStartIndex) + 1) * sizeof((inArray)[0]), (inShiftCount) * sizeof((inArray)[0])); \
    } while ( 0 )
...
#define ArrayCopy(dest, src, size) Mem_Copy_Forward(dest, src, size * sizeof((src)[0]))
...
Public u32 FindAnyByteForwardTimes(void* mem, void* end, void* bytesToFind, u32 sizeBytes, u32 timesToIgnore) {
    u32 timesLeft = timesToIgnore;
...
Public b8 Mem_Comp_Zero(void* mem, u32 size) {
    byte* scan = (byte*)mem;
